...
namespace PeerConnectionClient.Signalling
{
    ...
    public class Conductor
    {
		...
		{
			if (string.IsNullOrEmpty(codeUri) || string.IsNullOrEmpty(pollUri))
			{
				return;
			}
		...
		{
			if (string.IsNullOrEmpty(turnCredentialUri))
			{
				return;
			}
        ...
        /// </summary>
        private void Signaller_OnServerConnectionFailure(Exception ex)
        {
            Debug.WriteLine("[Error]: Connection to server failed: " + ex.Message + "\r\n" + ex.StackTrace);
        }
