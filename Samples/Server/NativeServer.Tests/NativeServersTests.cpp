...
TEST(BufferCapturerTests, InitializeDirectXDeviceResources)
{
	...
	ASSERT_TRUE(deviceResources->GetD3DDevice());
	ASSERT_TRUE(deviceResources->GetD3DDeviceContext());
	ASSERT_FALSE(deviceResources->GetSwapChain());
	ASSERT_FALSE(deviceResources->GetBackBufferRenderTargetView());
}
...
TEST(BufferCapturerTests, CaptureFrameUsingDirectXBufferCapturer)
{
	...
	ASSERT_TRUE(capturer->staging_frame_buffer_.Get());
