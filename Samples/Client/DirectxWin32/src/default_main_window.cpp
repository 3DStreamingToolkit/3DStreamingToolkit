...
void DefaultMainWindow::CreateChildWindows()
{
	...
	CreateChildWindow(&auth_uri_, AUTH_ID, L"Edit", ES_LEFT , 0);

	CreateChildWindow(&auth_code_label_, AUTH_ID, L"Static", ES_CENTER | ES_READONLY, 0);
	CreateChildWindow(&auth_code_, AUTH_ID, L"Edit", ES_LEFT  , 0);
...
{
	auth_code_val_ = str;
	::SetWindowText(auth_code_, auth_code_val_.c_str());
}

void DefaultMainWindow::SetAuthUri(const std::wstring& str)
{
	auth_uri_val_ = str;
	::SetWindowText(auth_uri_, auth_uri_val_.c_str());
}
