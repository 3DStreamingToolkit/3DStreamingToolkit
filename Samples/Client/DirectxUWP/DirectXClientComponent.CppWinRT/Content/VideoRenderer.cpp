...
winrt::Windows::Foundation::IAsyncAction VideoRenderer::CreateDeviceDependentResources()
{
	...
	auto psFile = co_await DX::ReadDataAsync(L"PixelShader.cso");
	//loadPixelShaderTask.then([this](std::vector<byte> file)
	//{
		m_deviceResources->GetD3DDevice()->CreatePixelShader(
			psFile.data(), psFile.size(), nullptr, &m_pixelShader);
	...
	if (!m_usingVprtShaders)
	{
		...
		auto gsFile = co_await DX::ReadDataAsync(L"GeometryShader.cso");/* .then(*/
